...
const toPublicNode = (
	...
	...(presentation.minPresenceAhead === undefined
		? {}
		: { minPresenceAhead: presentation.minPresenceAhead }),
	...(presentation.orphans === undefined
		? {}
		: { orphans: presentation.orphans }),
...
const projectionFingerprints = async (
	data: ResumeData,
	languageVersion?: number,
): Promise<ProjectionFingerprints> => ({
	formatVersion: PUBLIC_STYLE_PROJECTION_FORMAT_VERSION,
	languageVersion:
		languageVersion ??
		(data.metadata.stylesheet?.mode === "semantic"
			? data.metadata.stylesheet.applied.languageVersion
			: 1),
	semanticTreeVersion: SEMANTIC_TREE_VERSION,
...
export async function createPublicStyleProjection(input: {
	data: ResumeData;
	...
	const structure = projectNodeStructure(
		runtime.sourceTree,
		runtime.renderTree,
	);
...
export async function validatePublicStyleProjection(
	...
	if (
		!SUPPORTED_SEMANTIC_CSS_VERSIONS.includes(projection.languageVersion as 1)
	)
		return false;
	if (
		data.metadata.stylesheet?.mode === "semantic" &&
		projection.languageVersion !==
			data.metadata.stylesheet.applied.languageVersion
	) {
		return false;
	}
	const projectionData = dataForPublicProjection(
		data,
		projection.languageVersion,
	);
	const expected = await projectionFingerprints(
		projectionData,
		projection.languageVersion,
	);
	if (
	...
	try {
		return (
			projection.renderDataHash ===
			(await hashProjection(projectionData, projection.nodes, expected))
		);
	} catch {
		return false;
	}
	...
	Object.freeze(
		Object.fromEntries(
			Object.entries(nodes).map(
				([
					nodeKey,
					{ hidden: _hidden, order: _order, style, ...presentation },
				]) => [
					nodeKey,
					{
						...presentation,
...
const applyProjectedStructure = (
	...
	children: node.children
		.map((child, sourceIndex) => ({
			child,
			sourceIndex,
			structure: nodes[child.key],
		}))
		.filter(({ structure }) => !structure?.hidden)
		.sort(
			(left, right) =>
				(left.structure?.order ?? left.sourceIndex) -
					(right.structure?.order ?? right.sourceIndex) ||
