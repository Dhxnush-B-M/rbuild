...
const fixtureUrl = (name: string) =>
	new URL(`./__fixtures__/legacy/${name}`, import.meta.url);
const readFixture = (name: string): unknown =>
	JSON.parse(readFileSync(fixtureUrl(`${name}.json`), "utf8"));
const readExpected = (name: string): string =>
	readFileSync(fixtureUrl(`${name}.expected.css`), "utf8");
...
describe("convertLegacyStyleRules", () => {
	it("preserves legacy specificity and source-order ties deterministically", () => {
		const converted = convertLegacyStyleRules(
			dataWithRules("merge-specificity"),
		);
		...
		expect(converted.sanitizedRules.map(({ id }) => id)).toEqual([
			"id",
			"type",
			"global",
		]);
		expect(tie.source.text.indexOf("/* First */")).toBeLessThan(
			tie.source.text.indexOf("/* Second */"),
		);
	...
	it("sanitizes #3199 intent data and applies the existing PDF clamps", () => {
		const sanitized = convertLegacyStyleRules(
			dataWithRules("sanitized-intent-3199"),
		);
		const clamped = convertLegacyStyleRules(dataWithRules("clamped-spacing"));

		expect(sanitized.sanitizedRules).toEqual(
			styleRulesSchema.parse(readFixture("sanitized-intent-3199")),
		);
		expect(sanitized.source.text).toContain("color: #123456;");
		expect(sanitized.source.text).not.toMatch(
			/huge|unknown-property|unknown-slot|#ffffff/i,
		);
	...
	it("comments disabled and final-host no-effect declarations without inventing @resume-disabled", () => {
		const disabled = convertLegacyStyleRules(dataWithRules("disabled-rules"))
			.source.text;
		const links = convertLegacyStyleRules(dataWithRules("link-underline-3134"))
			.source.text;
	...
	it("maps every rich-text slot to its real semantic host", () => {
		const source = convertLegacyStyleRules(dataWithRules("rich-text-all-slots"))
			.source.text;
		...
		expect(
			compileStylesheet({ languageVersion: 1, text: source }).program,
		).not.toBeNull();
	});

	it("preserves Bold-after-text #3146 while retaining the award unbold exception", () => {
		const primary = convertLegacyStyleRules(
			dataWithRules("primary-text-bold-3146"),
		).source.text;
		const award = convertLegacyStyleRules(dataWithRules("award-unbold")).source
			.text;
		...
		expect(primary).toContain(
			'resume[template="scizor"] section[type="experience"]',
		);
	...
	it("applies legacy text weight to a nested role position without overriding real Bold hosts", () => {
		const source = convertLegacyStyleRules(
			dataWithRules("primary-text-bold-3146"),
		).source.text;

		expect(source).toContain(
			'item[role~="nested-role"] > item-header > field[name="position"]',
		);
	...
	it("emits legacy text declarations for the combined outer Text without widening field selectors", () => {
		const source = convertLegacyStyleRules(dataWithRules("combined-text-host"))
			.source.text;
	...
	it("translates icon and level font sizes to explicit geometry", () => {
		const source = convertLegacyStyleRules(dataWithRules("icon-level-size"))
			.source.text;

		expect(source).toMatch(/icon \{[\s\S]*height: 18pt;[\s\S]*width: 18pt;/);
		expect(source).toMatch(/level \{[\s\S]*font-size: 14pt;/);
		expect(source).toMatch(
			/level > icon \{[\s\S]*height: 14pt;[\s\S]*width: 14pt;/,
		);
	});

	it("uses portable custom-section types and quoted UUID section IDs", () => {
		const custom = convertLegacyStyleRules(dataWithRules("custom-section-type"))
			.source.text;
		const uuid = convertLegacyStyleRules(dataWithRules("section-id-uuid"))
			.source.text;

		expect(custom).toContain(
			'section[type="experience"] > section-items > item',
		);
		expect(uuid).toContain(
			'section[id="1d7312cb-9ba2-4d42-9ca8-2a9ca05f9f37"] > section-heading',
		);
	});

	it("compiles the all-template portable smoke fixture", () => {
		const result = convertLegacyStyleRules(
			dataWithRules("all-templates-smoke"),
		);
		expect(
			compileStylesheet(result.source).diagnostics.filter(
