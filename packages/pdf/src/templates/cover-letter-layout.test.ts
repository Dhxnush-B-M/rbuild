...
describe("cover letter PDF layout", () => {
	...
	it.each(pageFiles)("%s renders the shared page props", (file) => {
		...
		expect(source, basename(file)).toContain(
			"pageSize, pageMinHeightStyle, showHeader, pageNumber",
		);
		expect(source, basename(file)).toContain(
		...
		expect(source, basename(file)).not.toContain(
			'from "../shared/cover-letter"',
		);
