...
const isCoverLetterSection = (data: ResumeData, sectionId: string) =>
	data.customSections.some(
		(section) => section.id === sectionId && section.type === "cover-letter",
	);
...
const isCoverLetterOnlyDocument = (data: ResumeData) => {
	...
	return (
		...
		visibleSections.every((sectionId) => isCoverLetterSection(data, sectionId))
	);
};

export const shouldShowResumeHeader = (
	data: HeaderResumeData,
	pageIndex: number,
) =>
	pageIndex === 0 &&
