...
const contactChildNodeKey = (
	...
	if (typeof props.partKey === "string")
		return semanticTemplatePartNodeKey(contactListNodeKey, props.partKey);
	if ("email" in props)
		return semanticNodeKeys.contactItem(contactListNodeKey, "email");
	if ("phone" in props)
		return semanticNodeKeys.contactItem(contactListNodeKey, "phone");
	if ("location" in props)
		return semanticNodeKeys.contactItem(contactListNodeKey, "location");
	if ("website" in props)
		return semanticNodeKeys.contactItem(contactListNodeKey, "website");
	if (
...
const projectContactChildren = (
	...
	return entries.length === authored.length
		? projectRenderedChildren(renderedChildKeys, entries)
		: authored;
};
...
const usePrimitiveNodeKey = ({
	...
	if (semanticField && parentKey)
		return semanticNodeKeys.field(parentKey, semanticField);
	if (!parentKey) return undefined;

	if (parentKey.endsWith("/header")) {
		if (heading && children === data.basics.name)
			return semanticNodeKeys.headerPart(parentKey, "name");
		if (!heading && children === data.basics.headline)
			return semanticNodeKeys.headerPart(parentKey, "headline");
	}

	if (
		heading &&
		parentKey.includes("/section-") &&
		!parentKey.includes("/section-items")
	) {
...
export const Div = ({
	...
	return (
		<View
			...
			style={composeStyles(
				divStyle,
				style as Style | Style[] | undefined,
				resolved.style,
			)}
		/>
	);
...
export const Link = ({
	...
	const resolvedNodeKey =
		nodeKey ??
		(semanticRole && parentKey
			? semanticNodeKeys.link(parentKey, semanticRole)
			: undefined) ??
		(semanticField && parentKey
			? semanticNodeKeys.field(parentKey, semanticField)
			: undefined);
	const resolved = useResolvedNode(resolvedNodeKey);
	const visible = useSemanticNodeVisible(resolvedNodeKey);
	if (!visible) return null;
...
export const Icon = ({
	...
	const {
		...
		size: templateSize,
		...iconProps
	} = useTemplateIconSlot("icon");
	const iconRuleStyle = useSectionStyleRule("icon");
	const composedStyle = composeStyles(
		asStyleInput(iconStyle),
		iconRuleStyle,
		asStyleInput(style),
	);
	const templateIconSize =
		typeof templateSize === "number" || typeof templateSize === "string"
			? templateSize
			: undefined;
	const parentKey = useSemanticNodeKey();
	const resolvedNodeKey =
		nodeKey ??
		(parentKey ? semanticNodeKeys.icon(parentKey, "item") : undefined);
	const resolved = useResolvedNode(resolvedNodeKey);
	const visible = useSemanticNodeVisible(resolvedNodeKey);
	const resolvedSize =
		resolveIconSize({
...
export const SemanticHeaderView = ({
	style,
	...props
...
export const SemanticRegionView = ({
	region,
	style,
	...props
}: ComponentProps<typeof View> & { region: string }) => {
	const pageNodeKey = useTemplatePageNodeKey();
	...
	return (
		<View
			{...props}
			{...resolvedPdfFlowProps(resolved)}
			style={composeStyles(asStyleInput(style), resolved.style)}
		/>
	);
};
...
export const SemanticContactListView = ({
	style,
	...props
}: ComponentProps<typeof View>) => {
	const headerNodeKey = useSemanticNodeKey();
	const nodeKey = headerNodeKey
	...
	if (!visible) return null;
	const children =
		nodeKey && renderedChildKeys
			? projectContactChildren(props.children, nodeKey, renderedChildKeys)
			: props.children;
...
export function semanticTemplatePartNodeKey(
	ownerNodeKey: string | undefined,
	...partKeys: string[]
) {
	return ownerNodeKey
		? partKeys.reduce(
				(key, part) => `${key}/template-part-${part}`,
				ownerNodeKey,
			)
		: undefined;
}
...
export const SemanticTemplatePartView = ({
	...
	const nodeKey = semanticTemplatePartNodeKey(
		ownerNodeKey ?? contextualOwnerNodeKey,
		...partKeys,
	);
	const resolved = useResolvedNode(nodeKey);
	const visible = useSemanticNodeVisible(nodeKey);
...
export const SemanticHeaderPicture = ({
	style,
	...props
	...
	const headerNodeKey =
		useSemanticNodeKey() ??
		semanticNodeKeys.header(semanticNodeKeys.region(pageNodeKey, "header"));
	const nodeKey = headerNodeKey
		? semanticNodeKeys.headerPart(headerNodeKey, "picture")
		: undefined;
	const resolved = useResolvedNode(nodeKey);
	const visible = useSemanticNodeVisible(nodeKey);
	if (!visible) return null;
...
export const SectionHeadingIcon = ({
	...
	const { style: fallbackIconStyle, ...fallbackIconProps } =
		useTemplateIconSlot("icon");

	// Fall back to the item icon slot if no section heading icon slot is defined
	const hasSlot =
		sectionIconStyle !== undefined || Object.keys(sectionIconProps).length > 0;
