...
function isCoverLetterOnlyDocument(data: ResumeData): boolean {
	...
	return (
		sectionIds.length > 0 &&
		sectionIds.every((sectionId) => isCoverLetterSection(data, sectionId))
	);
}
...
export function buildMarkdown(
	...
	const blocks: string[] = isCoverLetterOnlyDocument(data)
		? []
...
function renderSection(
	...
	if (sectionId in data.sections) {
		...
		return section
			? renderBuiltInSection(sectionId as SectionType, titled(section))
			: [];
	}
...
function renderHeader(basics: ResumeData["basics"]): string[] {
	...
	const contact = [
		...
		...basics.customFields.map((field) =>
			field.link ? link(field.text, field.link) : field.text,
		),
	].filter(Boolean);
...
function visibleItems<T extends { hidden: boolean }>(section: {
	hidden: boolean;
	items: T[];
}): T[] {
...
function renderSummary(summary: ResumeData["summary"]): string[] {
	...
	return [heading(summary.title), htmlToMarkdown(summary.content)].filter(
		Boolean,
	);
}
	...
	for (const item of items) {
		if (item.roles.length > 0) {
			blocks.push(
				...
				italicLine([item.location]),
			);
			for (const role of item.roles) {
				blocks.push(
					italicLine([role.position, role.period]),
					htmlToMarkdown(role.description),
				);
			}
		} else {
			blocks.push(
...
function renderEducation(section: Sections["education"]): string[] {
	...
	for (const item of items) {
		...
		blocks.push(
			italicLine([item.location, item.grade ? `Grade: ${item.grade}` : ""]),
		);
		blocks.push(
			htmlToMarkdown(item.description),
			link(item.website.label, item.website.url),
		);
	}
	return blocks.filter(Boolean);
}
...
function renderDatedEntries(
	...
	for (const item of items) {
		...
		blocks.push(
			htmlToMarkdown(entry.description),
			link(entry.website.label, entry.website.url),
		);
	}
	return blocks.filter(Boolean);
}
...
function renderList(
	...
	for (const item of items) {
		...
		if (entry.keywords && entry.keywords.length > 0)
			line += `: ${entry.keywords.join(", ")}`;
		if (entry.website?.url)
			line += ` (${link(entry.website.label || entry.website.url, entry.website.url)})`;
		lines.push(line);
	}
...
const sectionRenderers: Record<
	SectionType,
	(section: Sections[SectionType]) => string[]
> = {
	...
	skills: (s) =>
		renderList(
			s as Sections["skills"],
			(item: Sections["skills"]["items"][number]) => ({
				label: item.name,
	...
	languages: (s) =>
		renderList(
			s as Sections["languages"],
			(item: Sections["languages"]["items"][number]) => ({
				label: item.language,
				detail: item.fluency,
			}),
		),
	interests: (s) =>
		renderList(
			s as Sections["interests"],
			(item: Sections["interests"]["items"][number]) => ({
				label: item.name,
				keywords: item.keywords,
			}),
		),
	profiles: (s) =>
		renderList(
			s as Sections["profiles"],
			(item: Sections["profiles"]["items"][number]) => ({
				label: item.network,
	...
	references: (s) =>
		renderList(
			s as Sections["references"],
			(item: Sections["references"]["items"][number]) => ({
				label: item.name,
	...
	awards: (s) =>
		renderDatedEntries(
			s as Sections["awards"],
			(item: Sections["awards"]["items"][number]) => ({
				title: item.title,
	...
	certifications: (s) =>
		renderDatedEntries(
			s as Sections["certifications"],
			(item: Sections["certifications"]["items"][number]) => ({
				title: item.title,
	...
	publications: (s) =>
		renderDatedEntries(
			s as Sections["publications"],
			(item: Sections["publications"]["items"][number]) => ({
				title: item.title,
	...
	volunteer: (s) =>
		renderDatedEntries(
			s as Sections["volunteer"],
			(item: Sections["volunteer"]["items"][number]) => ({
	...
	if (sectionType === "summary") {
		...
		for (const item of items)
			if ("content" in item && item.content)
				blocks.push(htmlToMarkdown(item.content));
		return blocks.filter(Boolean);
	}
	...
	if (sectionType === "cover-letter") {
		...
		for (const item of items) {
			if ("recipient" in item && item.recipient)
				blocks.push(htmlToMarkdown(item.recipient));
			if ("content" in item && item.content)
				blocks.push(htmlToMarkdown(item.content));
		}
		return blocks.filter(Boolean);
