...
describe("htmlToMarkdown", () => {
	it("converts the constrained tiptap tag set", () => {
		const html =
			"<p>Led <strong>teams</strong> of <em>engineers</em>.</p><ul><li>Shipped X</li><li>Owned Y</li></ul>";
		expect(htmlToMarkdown(html)).toBe(
			"Led **teams** of _engineers_.\n\n- Shipped X\n- Owned Y",
		);
	});

	it("converts anchors to Markdown links and decodes entities", () => {
		expect(
			htmlToMarkdown('<p>See <a href="https://x.com">Tom &amp; Jerry</a></p>'),
		).toBe("See [Tom & Jerry](https://x.com)");
	});
...
describe("buildMarkdown", () => {
	...
	const md = buildMarkdown(
		getResumeExportData(sampleResumeData, "resume"),
		resolveTitle,
	);
	...
	it("excludes the cover letter from the resume scope and includes it in the cover-letter scope", () => {
		const cover = buildMarkdown(
			getResumeExportData(sampleResumeData, "cover-letter"),
		);
