...
function flatten(root: SemanticNode): SemanticNode[] | null {
	...
	while (stack.length > 0) {
		...
		if (
			childCount >
			SEMANTIC_CSS_LIMITS_V1.maxSemanticNodes - nodes.length - stack.length
		)
			return null;
...
export function analyzeStylesheet(
	...
	for (const rule of program.rules) {
		const matches = nodes.filter((node) =>
			matchesSelector(rule.selector, node.key),
		);
		if (matches.length === 0) {
			diagnostics.push(
				createDiagnostic(
		...
		for (const declaration of rule.declarations) {
			...
			if (
				definition &&
				!matches.some((node) => definition.appliesTo.includes(node.kind))
			) {
				diagnostics.push(
					createDiagnostic(
