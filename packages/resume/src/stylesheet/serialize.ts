...
function escapeCssValue(value: string | number): string {
	...
	if (text.length === 0 || /[\0;{}]|\/\*/.test(text))
		throw new Error("unsafe CSS declaration value");
	return text;
}

export function serializeGeneratedStylesheet(
	stylesheet: GeneratedStylesheet,
): string {
	if (
		!Number.isSafeInteger(stylesheet.languageVersion) ||
		stylesheet.languageVersion < 1
	) {
	...
	const blocks = stylesheet.blocks.map((block) => {
		...
		if (!selector.selector)
			throw new Error(
				`unsafe generated Semantic CSS selector: ${selector.error}`,
			);
		...
		const declarations = Object.entries(block.declarations)
			...
			.map(
				([property, value]) =>
					`\t${escapeCssIdentifier(kebabCase(property))}: ${escapeCssValue(value)};`,
			)
			.join("\n");
		if (declarations.length === 0)
			throw new Error(
				"generated Semantic CSS blocks require at least one declaration",
			);

		const comment =
			block.comment === undefined
				? ""
				: `/* ${escapeCssComment(block.comment)} */\n`;
