...
function assertEnumerableDataProperty(
	...
	if (!descriptor?.enumerable || "get" in descriptor || "set" in descriptor) {
		throw new Error(
			"I-JSON objects must contain only enumerable data properties",
		);
	}
}
...
function assertIJsonValue(value: unknown, seen = new Set<object>()): void {
	...
	if (typeof value === "string") {
		if (hasUnpairedSurrogate(value))
			throw new Error("I-JSON values must not contain unpaired surrogates");
		return;
	}
	...
	if (typeof value !== "object")
		throw new Error(
			"I-JSON values must be JSON primitives, arrays, or plain objects",
		);
	...
	try {
		if (Array.isArray(value)) {
			for (const symbol of Object.getOwnPropertySymbols(value)) {
				throw new Error(
					`I-JSON arrays must not contain symbol keys: ${String(symbol)}`,
				);
			}
			...
			for (let index = 0; index < value.length; index++) {
				...
				if (!descriptor)
					throw new Error("I-JSON arrays must not contain holes");
				assertIJsonValue(descriptor.value, seen);
			}
		} else {
			for (const symbol of Object.getOwnPropertySymbols(value)) {
				throw new Error(
					`I-JSON objects must not contain symbol keys: ${String(symbol)}`,
				);
			}
			for (const key of Object.getOwnPropertyNames(value)) {
				if (hasUnpairedSurrogate(key))
					throw new Error("I-JSON keys must not contain unpaired surrogates");
...
export async function computeRenderDataHash(
	input: RenderDataHashInput,
): Promise<string> {
	if (input.domainVersion !== HASH_DOMAIN_VERSION) {
		throw new Error(
			`Unsupported public render hash domain version: ${input.domainVersion}`,
		);
	}

	const payload =
		input.resolvedNodes === undefined &&
		input.projectionFingerprints === undefined
			...
			: {
					...
					...(input.resolvedNodes === undefined
						? {}
						: { resolvedNodes: input.resolvedNodes }),
					...(input.projectionFingerprints === undefined
						? {}
	...
	if (canonical === undefined)
		throw new Error("I-JSON value required for public render hashing");
