...
export type PropertyRegistry = Readonly<
	Record<string, PropertyDefinition | undefined>
>;

export const SEMANTIC_CSS_CSS_WIDE_KEYWORDS_V1 = [
	"inherit",
	"initial",
	"revert",
	"unset",
] as const;
export const SEMANTIC_CSS_LENGTH_UNITS_V1 = [
	"pt",
	"px",
	"in",
	"mm",
...
export const SEMANTIC_CSS_BORDER_STYLE_VALUES_V1 = [
	"dotted",
	"dashed",
	"solid",
] as const;
...
const structuralNodes = [
	...SEMANTIC_NODE_KINDS.filter((kind) => kind !== "resume"),
] as SemanticNodeKind[];
const lengthProperties = new Set<string>(SEMANTIC_CSS_LENGTH_PROPERTIES_V1);
const numericLengthUnits = SEMANTIC_CSS_LENGTH_UNITS_V1.filter(
	(unit) => unit !== "%",
);
...
function propertyUnits(name: string): readonly string[] {
	if (name === "size" || name === "-resume-min-presence-ahead")
		return numericLengthUnits;
	return lengthProperties.has(name) || name === "line-height"
...
const properties = {
	...
	...entries(["text-transform"], {
		category: "text",
		inheritable: true,
		appliesTo: textAndLinkNodes,
	}),
	...entries(["vertical-align"], {
		category: "text",
		inheritable: false,
		appliesTo: textAndLinkNodes,
	}),
