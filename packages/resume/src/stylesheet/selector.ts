...
type CompiledSimpleSelector =
	...
	| {
			type: "pseudo";
			name: "root" | "first-child" | "last-child" | "only-child";
	  }
	| {
			type: "pseudo";
...
const knownRoles = new Set(
	Object.values(SEMANTIC_REGISTRY_V1).flatMap((definition) => definition.roles),
);
...
function astName(node: SelectorAst): string {
	...
	if (node.name && typeof node.name.name === "string")
		return identifier(node.name.name);
...
function validateCompound(selectors: readonly CompiledSimpleSelector[]): void {
	const types = selectors.filter(
		(selector): selector is Extract<CompiledSimpleSelector, { type: "type" }> =>
			selector.type === "type",
	);
	if (types.length > 1)
		throw new Error("A compound selector can contain only one type selector.");
	...
	for (const selector of selectors) {
		...
		if (selector.name === "role") {
			if (
				!roleValueIsKnown(selector.matcher, selector.value, allowedRoles(type))
			) {
		...
		if (
			type &&
			!(SEMANTIC_REGISTRY_V1[type].attributes as readonly string[]).includes(
				selector.name,
			)
		) {
			throw new Error(
...
function compileNth(
	...
	if (expression.type === "Identifier") {
		...
		a =
			expression.a === null || expression.a === undefined
				? 0
				: Number(expression.a);
		b =
			expression.b === null || expression.b === undefined
				? 0
				: Number(expression.b);
		if (!Number.isInteger(a) || !Number.isInteger(b))
			throw new Error(`Invalid :${name} expression.`);
	} else {
		throw new Error(`Unsupported :${name} expression.`);
	}

	if (name === "nth-of-type" && nth[0].selector)
		throw new Error(":nth-of-type does not accept an of selector.");
	const of = nth[0].selector
		? compileSelectorList(nth[0].selector, { depth: context.depth + 1 })
		: undefined;
...
function compileSimple(
	node: SelectorAst,
	context: CompileContext,
): CompiledSimpleSelector | null {
	switch (node.type) {
		case "TypeSelector": {
			const name = astName(node);
			if (name === "*") return { type: "universal" };
			if (!knownKinds.has(name))
				throw new Error(`Unknown semantic element ${name}.`);
		...
		case "AttributeSelector": {
			...
			if (!knownAttributes.has(name))
				throw new Error(`Unknown semantic attribute ${name}.`);
			if (node.flags)
				throw new Error("Attribute selector flags are not supported.");
		...
		case "PseudoClassSelector": {
			...
			if (["root", "first-child", "last-child", "only-child"].includes(name)) {
				...
				return {
					type: "pseudo",
					name: name as "root" | "first-child" | "last-child" | "only-child",
				};
			}
			if (["is", "where", "not"].includes(name)) {
				...
				const selectors = compileSelectorList(nested[0], {
					depth: context.depth + 1,
				});
				return {
					type: "pseudo",
					name: name as "is" | "where" | "not",
					selectors,
				};
			}
...
function compileComplex(
	...
	for (const child of childrenOf(node)) {
		if (child.type === "Combinator") {
			...
			if (
				combinators.length > SEMANTIC_CSS_LIMITS_V1.maxCombinatorsPerSelector
			) {
	...
	if (selectors.length === 0 && compounds.length > 0)
		throw new Error("Selector cannot end with a combinator.");
	validateCompound(selectors);
	compounds.push({ selectors });
	...
	return {
		compounds,
		combinators,
		specificity: [specificity[0], specificity[1], specificity[2]],
	};
}
...
function compileSelectorList(
	node: SelectorAst,
	context: CompileContext,
): readonly CompiledComplexSelector[] {
	if (node.type !== "SelectorList")
		throw new Error("Expected a SelectorList AST.");
	const selectors = childrenOf(node);
	if (
		selectors.length === 0 ||
		selectors.length > SEMANTIC_CSS_LIMITS_V1.maxSelectorsPerRule
	) {
	...
	try {
		...
		const ast = (
			typeof source === "string"
				? csstree.parse(source, { context: "selectorList", positions: true })
				: source
		) as SelectorAst;
...
function attribute(node: SemanticNode, name: string): string | undefined {
	...
	if (name === "role")
		return node.roles.length > 0 ? node.roles.join(" ") : undefined;
	...
	switch (selector.type) {
		...
		case "pseudo":
			switch (selector.name) {
				...
				case "is":
				case "where":
					return selector.selectors.some((nested) =>
						matchesComplex(nested, target),
					);
				case "not":
					return selector.selectors.every(
						(nested) => !matchesComplex(nested, target),
					);
				case "nth-child":
				case "nth-of-type": {
					...
					if (selector.name === "nth-of-type") {
						values = values.filter(
							(sibling) => sibling.node.kind === target.node.kind,
						);
					}
					if (selector.of)
						values = values.filter((sibling) =>
							selector.of?.some((nested) => matchesComplex(nested, sibling)),
						);
...
function matchesCompound(
	compound: CompiledCompoundSelector,
	target: TreeNode,
): boolean {
	return compound.selectors.every((selector) =>
		matchesSimple(selector, target),
	);
}

function matchesComplexAt(
	selector: CompiledComplexSelector,
	...
	switch (selector.combinators[compoundIndex - 1]) {
		case ">":
			return target.parent
				? matchesComplexAt(selector, compoundIndex - 1, target.parent)
				: false;
		case " ": {
			let ancestor = target.parent;
			while (ancestor) {
				if (matchesComplexAt(selector, compoundIndex - 1, ancestor))
					return true;
		...
		case "~": {
			...
			return values
				.slice(0, index)
				.some((sibling) =>
					matchesComplexAt(selector, compoundIndex - 1, sibling),
				);
		}
		default:
			return false;
...
export function createSelectorMatcher(
	root: SemanticNode,
): (selector: CompiledSelector, nodeKey: string) => boolean {
	const nodes = buildTree(root);
	return (selector, nodeKey) => {
		const target = nodes.get(nodeKey);
		return target
			? selector.selectors.some((complex) => matchesComplex(complex, target))
			: false;
	};
}
