...
describe("compileStylesheet", () => {
	...
	it("warns when version-one source omits the directive", () => {
		...
		expect(result.diagnostics).toContainEqual(
			expect.objectContaining({
				code: "MISSING_VERSION_DIRECTIVE",
				severity: "warning",
			}),
		);
	});
	...
	it("rejects duplicate directives", () => {
		...
		expect(result.diagnostics).toContainEqual(
			expect.objectContaining({
				code: "DUPLICATE_VERSION_DIRECTIVE",
				severity: "error",
			}),
		);
	});
	...
	it("rejects malformed directives", () => {
		...
		expect(result.program).toBeNull();
		expect(result.diagnostics).toContainEqual(
			expect.objectContaining({ code: "INVALID_VERSION", severity: "error" }),
		);
	});
	...
	it("does not compile a recovered CSS error", () => {
		...
		expect(result.program).toBeNull();
		expect(result.diagnostics).toContainEqual(
			expect.objectContaining({ code: "CSS_PARSE_ERROR", severity: "error" }),
		);
	});
});
