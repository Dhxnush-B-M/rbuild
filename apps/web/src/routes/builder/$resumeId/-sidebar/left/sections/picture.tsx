...
function PictureSectionForm() {
	...
	const uploadPictureFile = async (file: File) => {
		const toastId = toast.loading(t`Uploading picture…`);

		try {
			const publicUrl = await uploadPictureToSupabase(file);
			if (publicUrl) {
				form.setFieldValue("url", publicUrl);
				handleAutoSave();
				...
				reader.onload = (e) => {
					...
					form.setFieldValue("url", dataUrl);
					handleAutoSave();
					toast.dismiss(toastId);
				};
