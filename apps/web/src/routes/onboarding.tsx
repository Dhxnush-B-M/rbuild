...
function OnboardingPage() {
	...
	// Step 1: Name is manual from user input, not auto-overwritten
	const [fullName, setFullName] = useState(() => {
		if (typeof window !== "undefined") {
			return sessionStorage.getItem("rbuilder_onboarding_name") || "";
		}
		return "";
	});
	const [phone, setPhone] = useState(() => {
		if (typeof window !== "undefined") {
			const saved = sessionStorage.getItem("rbuilder_onboarding_phone") || "";
			return saved.replace(/^\+91\s*/, "");
		}
		return "";
	});
	...
	// Check if returning from payment synchronously to prevent any page blink/glitch
	const [isVerifyingReturn, setIsVerifyingReturn] = useState(() => {
		if (typeof window !== "undefined") {
			const sp = new URLSearchParams(window.location.search);
			return Boolean(
				sp.get("payment_status") ||
				sp.get("razorpay_payment_id") ||
				sp.get("razorpay_payment_link_status") === "paid" ||
				sp.get("razorpay_payment_link_id")
			);
		}
		return false;
	});
	...
	useEffect(() => {
			...
			if (
				...
				void getCurrentSupabaseUser().then(async (profile) => {
					...
					const savedName =
						...
						"User";
					const rawPhone =
						sessionStorage.getItem("rbuilder_onboarding_phone") ||
						profile?.phone ||
						phone.trim() ||
						"";
					const savedPhone = rawPhone.startsWith("+")
						? rawPhone
						: rawPhone
							? `${countryCode} ${rawPhone}`
							: "";
					const savedPlanId =
						(sessionStorage.getItem("rbuilder_onboarding_plan") as
							| "1_month"
							| "3_months") || selectedPlan.id;
					const userEmail =
						profile?.email ||
						email ||
		...
		void getCurrentSupabaseUser().then((profile) => {
			...
			// Do NOT auto-override fullName with Google OAuth, let user enter manually
		});

		return () => {
			isMounted = false;
		};
	}, [navigate, countryCode, email, fullName, phone, selectedPlan]);
	...
	const handleNextStep = async () => {
		if (currentStep === 1) {
			...
			if (typeof window !== "undefined") {
				sessionStorage.setItem("rbuilder_onboarding_name", cleanName);
			}
			setCurrentStep(2);
			...
			if (typeof window !== "undefined") {
				sessionStorage.setItem("rbuilder_onboarding_phone", fullPhone);
			}

			// Immediately save to Supabase Database
			const userEmail =
	...
	const handleInitiatePayment = async (planOverride?: typeof selectedPlan) => {
		...
		const rawPhone =
			...
			(typeof window !== "undefined"
				...
				: "") ||
			"";
		const userPhone = rawPhone.startsWith("+")
			? rawPhone
			: rawPhone
				? `${countryCode} ${rawPhone}`
				: "";
		...
		if (typeof window !== "undefined") {
			sessionStorage.setItem("rbuilder_onboarding_name", userName);
			sessionStorage.setItem("rbuilder_onboarding_phone", userPhone);
			sessionStorage.setItem("rbuilder_onboarding_plan", planToPay.id);
		}

		// Ensure profile is synced to Supabase with manual name and phone before initiating payment
		if (userEmail) {
			await saveUserToSupabase({
				email: userEmail,
				name: userName,
	...
	// Dedicated Smooth Screen on Payment Return to prevent any blink/glitch
	if (isVerifyingReturn) {
		return (
			<div className="relative flex min-h-svh w-full flex-col items-center justify-center overflow-x-hidden bg-[#F8F9FD] px-4 py-8 dark:bg-[#0C0D18]">
				<div className="relative z-10 flex w-full max-w-[380px] flex-col items-center justify-center rounded-[32px] border border-white/80 bg-white p-8 text-center shadow-xl dark:border-white/10 dark:bg-[#151624]">
					<div className="relative mb-5 flex size-20 items-center justify-center rounded-full bg-purple-100 dark:bg-purple-950/50">
						<SparkleIcon
					...
					<p className="mt-2 text-muted-foreground text-xs">
						Verifying payment and setting up your workspace
					</p>
					<div className="mt-6 flex items-center gap-2 font-semibold text-purple-600 text-xs dark:text-purple-400">
			...
			>
				...
				<div className="mb-6 text-center">
					<div className="mb-3 flex items-center justify-center gap-2">
						<BrandLogoSvg className="size-7 shadow-purple-500/20 shadow-sm" />
							...
							return (
								<div key={s.num} className="flex flex-1 items-center">
									...
									{idx < stepsConfig.length - 1 && (
										<div
											className={`mx-1.5 h-0.5 flex-1 border-dashed border-t-2 ${
												currentStep > idx + 1
													? "border-purple-600"
						...
						>
							...
							>
								...
								<span>
									{isLoading
										? "Opening Razorpay..."
										: `Pay ₹${selectedPlan.amountInRupees} with Razorpay`}
								</span>
							</button>
