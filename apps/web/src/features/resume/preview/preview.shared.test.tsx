...
describe("ResumePreviewLoader", () => {
	it("uses the same scaled page dimensions as the preview page", () => {
		...
		const expectedSize = getScaledPreviewPageSize(
			DEFAULT_PDF_PAGE_SIZE,
			pageScale,
		);
		...
		expect(Number.parseFloat(page?.style.height ?? "0")).toBeCloseTo(
			expectedSize.height,
		);
		expect(Number.parseFloat(page?.style.width ?? "0")).toBeCloseTo(
			expectedSize.width,
		);
	});
