...
describe("Semantic CSS editor extensions", () => {
	it("uses only Semantic CSS registries and the current resume for completion", async () => {
		const selectorLabels = await getSemanticCssCompletionLabels(
			"",
			0,
			metadata,
		);
		const propertyLabels = await getSemanticCssCompletionLabels(
			"section {\n\tco",
			13,
			metadata,
		);
		const variableSource = "resume { --brand-accent: #f00; color: var(--br";
		const variableLabels = await getSemanticCssCompletionLabels(
			variableSource,
			variableSource.length,
			metadata,
		);
		const systemLabels = await getSemanticCssCompletionLabels(
			"--resume-",
			5,
			metadata,
		);
		const directiveLabels = await getSemanticCssCompletionLabels(
			"@",
			1,
			metadata,
		);
		...
		expect(variableLabels).toEqual(
			expect.arrayContaining(["--brand-accent", "--resume-primary-color"]),
		);
		...
		expect(directiveLabels).toEqual(
			expect.arrayContaining(["@media", "@version 1;"]),
		);
	});
	...
	it("offers only the current property's registered compiler vocabulary", () => {
		...
		const displayLabels = getSemanticCssCompletionLabels(
			displaySource,
			displaySource.length,
			metadata,
		);
		const borderStyleLabels = getSemanticCssCompletionLabels(
			borderStyleSource,
			borderStyleSource.length,
			metadata,
		);
		const fontSizeLabels = getSemanticCssCompletionLabels(
			fontSizeSource,
			fontSizeSource.length,
			metadata,
		);

		expect(displayLabels).toEqual(
			expect.arrayContaining(["flex", "none", "inherit"]),
		);
		expect(displayLabels).not.toEqual(
			expect.arrayContaining(["portrait", "dashed", "pt"]),
		);
		expect(borderStyleLabels).toEqual(
			expect.arrayContaining(["dashed", "dotted", "solid"]),
		);
		expect(borderStyleLabels).not.toContain("double");
		expect(fontSizeLabels).toEqual(expect.arrayContaining(["pt", "rem"]));
		expect(fontSizeLabels).not.toEqual(
			expect.arrayContaining(["none", "normal", "max-content"]),
		);
	});

	it.each(borderShorthands)(
		"offers complete %s shorthand values instead of bare units",
		(property) => {
			const source = `section { ${property}: `;
			const labels = getSemanticCssCompletionLabels(
				source,
				source.length,
				metadata,
			);
			...
			expect(labels).toEqual(
				expect.arrayContaining(["1pt dotted", "1pt dashed", "1pt solid"]),
			);
	...
	it("builds hover text from the same registries", () => {
		...
		const colorDocumentation = getSemanticCssHoverDocumentation(
			"color",
			metadata,
		);
		...
		expect(
			getSemanticCssHoverDocumentation("--resume-primary-color", metadata),
		).toMatch(/read-only.*builder primary color/i);
		expect(
			getSemanticCssHoverDocumentation("#section-experience", metadata),
		).toMatch(/current resume.*section/i);
		expect(
			getSemanticCssHoverDocumentation(
				'template-part[name="timeline-line"]',
				metadata,
			),
		).toMatch(/current template part/i);
	});
	...
	it("maps compiler offsets and only decorates compiler-confirmed color values", () => {
		...
		views.push(view);
		const swatches = view.dom.querySelectorAll<HTMLButtonElement>(
