...
const serializeZodCause = (
	cause: unknown,
): SerializedPreflightCause | undefined => {
	if (
...
const initialization = Promise.all([
	import("@rbuilder/pdf/preflight"),
	initializePdfInspection(),
] as const);
void initialization.then(() => self.postMessage({ type: "preflight_ready" }));

self.addEventListener(
	"message",
	async ({ data }: MessageEvent<PreflightWorkerRequest>) => {
		...
		if (!rendered.ok) {
			...
			result = failure(
				...
				"The PDF exceeds the preflight byte limit.",
			);
		} else {
			try {
				...
				result =
					pageCount > data.limits.maxPages
						? failure(
								...
								"The PDF exceeds the preflight page limit.",
							)
						: {
								ok: true,
								pageCount,
		...
		self.postMessage(response, {
			transfer: getPreflightTransferables(response),
		});
	},
);
