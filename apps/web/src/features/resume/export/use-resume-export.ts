...
import { buildDocx } from "@rbuilder/docx";
import { getResumeSectionTitle } from "@rbuilder/pdf/section-title";
import type { ResumeExportTarget } from "@rbuilder/resume/export-sections";
import {
	getResumeExportData,
	resumeHasCoverLetter,
} from "@rbuilder/resume/export-sections";
import { buildMarkdown } from "@rbuilder/resume/markdown";
import type { ResumeData } from "@rbuilder/schema/resume/data";
import type { SemanticStylesheet } from "@rbuilder/schema/resume/stylesheet";
import { downloadWithAnchor, generateFilename } from "@rbuilder/utils/file";
...
const createSectionTitleResolver = async (data: ResumeData) => {
	const resolveSectionTitle = await createSectionTitleResolverForLocale(
		data.metadata.page.locale,
	);
	const dataWithResolver = { ...data, resolveSectionTitle };
	return (sectionId: string) =>
		getResumeSectionTitle(dataWithResolver, sectionId);
};
...
const getExportName = (resume: ExportableResume) =>
	resume.name || resume.data.basics.name || resume.slug;
const getTargetExportName = (
	resume: ExportableResume,
	target: ResumeExportTarget,
) =>
	target === "cover-letter"
		? `${getExportName(resume)} Cover Letter`
		: getExportName(resume);
...
export function useResumeExport(resume: ExportableResume | undefined) {
	...
	const pdfPresentation = useMemo<ResumePdfPresentation | undefined>(
		() =>
			canonicalStylesheet
				? {
						stylesheet: {
							mode: canonicalStylesheet.mode,
							applied: canonicalStylesheet.applied,
						},
					}
				: undefined,
		[canonicalStylesheet],
	...
	const onDownloadMarkdown = useCallback(
		async (target: ResumeExportTarget = "resume") => {
			...
			const blob = new Blob([buildMarkdown(data, resolveTitle)], {
				type: "text/markdown",
			});
			downloadWithAnchor(
				blob,
				generateFilename(getTargetExportName(resume, target), "md"),
			);
		},
		[resume],
	);
	...
	const onDownloadDOCX = useCallback(
		async (target: ResumeExportTarget = "resume") => {
			...
			try {
				...
				downloadWithAnchor(
					blob,
					generateFilename(getTargetExportName(resume, target), "docx"),
				);
			} catch {
				toast.error(
					t`There was a problem while generating the DOCX, please try again.`,
				);
			}
		},
		[resume],
	...
	const onDownloadPDF = useCallback(
		async (
			...
			const toastId = toast.loading(
				t`Please wait while your PDF is being generated...`,
			);
			setIsExporting(true);
			try {
				const data = getResumeExportData(resume.data, target);
					...
					target === "cover-letter"
						? {
								includeCoverLetterHeader:
									downloadOptions?.includeCoverLetterHeader,
							}
						: undefined,
					pdfPresentation,
				);
				downloadWithAnchor(
					blob,
					generateFilename(getTargetExportName(resume, target), "pdf"),
				);
			} catch {
				toast.error(
					t`There was a problem while generating the PDF, please try again.`,
				);
			} finally {
	...
	const onPrint = useCallback(async () => {
		...
		try {
			...
			toast.error(
				t`There was a problem while preparing your resume for printing, please try again.`,
			);
		} finally {
			setIsExporting(false);
			toast.dismiss(toastId);
