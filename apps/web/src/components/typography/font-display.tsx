...
export function FontDisplay({ family, label, type, url }: FontDisplayProps) {
	...
	useEffect(() => {
		...
		const fontFace = new FontFace(previewName, `url(${url})`, {
			display: "swap",
		});
	...
	return (
		<div ref={containerRef} className="inline-flex items-baseline gap-2">
			<span
				style={{
					fontFamily: isLoaded ? `'${previewName}', sans-serif` : "sans-serif",
				}}
				className={cn(
			...
			{label !== family && (
				<span className="text-muted-foreground text-xs">{family}</span>
			)}
		</div>
	);
}
