...
describe("Combobox", () => {
	...
	it("places the clear action inside the trigger footprint", () => {
		const { container } = wrap(
			<Combobox options={[...options]} value="beta" showClear />,
		);
		...
		expect(
			container.querySelector("[data-slot='combobox-trigger'] > span"),
		).toHaveClass("pe-7");
	});
	...
	it("renders grouped options with group labels", async () => {
		...
		const groupedOptions: ComboboxOption[] = [
			{ value: "alpha", label: "Alpha", group: "Primary" },
			{
				value: "beta",
				label: "Beta",
				group: { value: "secondary", label: "Secondary" },
			},
			{
				value: "gamma",
				label: "Gamma",
	...
	it("renders nothing extra when given an empty options array (no crash)", () => {
		expect(() =>
			wrap(<Combobox options={[]} placeholder="Empty" />),
		).not.toThrow();
